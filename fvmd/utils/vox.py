...
class Vox_util(object):
    ...
    def Ref2Mem(self, xyz, Z, Y, X, assert_cube=False):
        ...
        xyz = fvmd.utils.geom.apply_4x4(mem_T_ref, xyz)
        return xyz
    ...
    def Mem2Ref(self, xyz_mem, Z, Y, X, assert_cube=False):
        ...
        xyz_ref = fvmd.utils.geom.apply_4x4(ref_T_mem, xyz_mem)
        return xyz_ref
    ...
    def get_mem_T_ref(self, B, Z, Y, X, assert_cube=False, device='cuda'):
        ...
        center_T_ref = fvmd.utils.geom.eye_4x4(B, device=device)
        ...
        mem_T_center = fvmd.utils.geom.eye_4x4(B, device=device)
        mem_T_center[:,0,0] = 1./vox_size_X
        mem_T_center[:,1,1] = 1./vox_size_Y
        mem_T_center[:,2,2] = 1./vox_size_Z
        mem_T_ref = fvmd.utils.geom.matmul2(mem_T_center, center_T_ref)
    ...
    def unproject_image_to_mem(self, rgb_camB, pixB_T_camA, camB_T_camA, Z, Y, X, assert_cube=False, xyz_camA=None):
        ...
        xyz_camB = fvmd.utils.geom.apply_4x4(camB_T_camA, xyz_camA)
        z = xyz_camB[:,:,2]

        xyz_pixB = fvmd.utils.geom.apply_4x4(pixB_T_camA, xyz_camA)
