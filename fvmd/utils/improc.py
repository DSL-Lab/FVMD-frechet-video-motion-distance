...
def reduce_emb(emb, valid=None, inbound=None, together=False):
    ...
    reduced_emb = fvmd.utils.basic.normalize(reduced_emb) - 0.5
...
def xy2heatmap(xy, sigma, grid_xs, grid_ys, norm=False):
    ...
    if norm:
        ...
        gauss_ = fvmd.utils.basic.normalize(gauss_)
...
def xy2heatmaps(xy, Y, X, sigma=30.0, norm=True):
    ...
    grid_y, grid_x = fvmd.utils.basic.meshgrid2d(B, Y, X, device=device)
...
def oned2inferno(d, norm=True, do_colorize=False):
    ...
    if norm:
        d = fvmd.utils.basic.normalize(d)
...
def draw_frame_id_on_vis(vis, frame_id, scale=0.5, left=5, top=20):
    ...
    frame_str = fvmd.utils.basic.strnum(frame_id)
    ...
    def summ_oneds(self, name, ims, frame_ids=None, bev=False, fro=False, logvis=False, reduce_max=False, max_val=0.0, norm=True, only_return=False, do_colorize=False):
        if self.save_this:
            ...
            if norm:
                ...
                im = fvmd.utils.basic.normalize(im)
        ...
        if self.save_this:
            ...
            if feats.ndim==6:
                ...
                if valids is None:
                    ...
                    feats = fvmd.utils.basic.reduce_masked_mean(feats, valids, dim=reduce_dim)
            ...
            if not pca:
                ...
                __p = lambda x: fvmd.utils.basic.pack_seqdim(x, B)
                __u = lambda x: fvmd.utils.basic.unpack_seqdim(x, B)
        ...
        if self.save_this:
            if feat.ndim==5: # B x C x D x H x W
                ...
                if valid is None:
                    ...
                    feat = fvmd.utils.basic.reduce_masked_mean(feat, valid, dim=reduce_axis)
